...
@api_view(["GET"])
def me(request):
    return Response({"id": request.user.id, "username": request.user.username, "character_count": request.user.characters.count(), "character_limit": 5})
...
def _prompt(character, history):
    ...
    return [{"role": "system", "content": f"你係一個以廣東話繁體中文對話嘅 AI 角色。模式：{mode}。角色名：{character.name}。背景：{character.description}。{grounding} 不索取密碼、地址、學校、電話或付款資料。"}, *history]

@api_view(["POST"])
def send_message(request, conversation_id):
    conversation = get_object_or_404(Conversation.objects.select_related("character"), id=conversation_id, character__owner=request.user)
    ...
    try:
        with httpx.Client(timeout=120) as client:
            response = client.post(f"{settings.OLLAMA_BASE_URL}/api/chat", json={"model": settings.CHAT_MODEL, "messages": _prompt(conversation.character, history), "stream": False})
            response.raise_for_status()
